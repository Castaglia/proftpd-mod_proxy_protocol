...
if (scalar(@ARGV) > 0) {
  ...
  my $FEATURE_TESTS = {
    ...
    't/modules/mod_proxy_protocol/sftp.t' => {
      order => ++$order,
      test_class => [qw(mod_proxy_protocol mod_sftp)],
    },

    't/modules/mod_proxy_protocol/tls.t' => {
      order => ++$order,
      test_class => [qw(mod_proxy_protocol mod_tls)],
    },

    't/modules/mod_proxy_protocol/wrap2.t' => {
      order => ++$order,
      test_class => [qw(mod_proxy_protocol mod_wrap2)],
    },
